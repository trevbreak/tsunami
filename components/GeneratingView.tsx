  ...
  return (
    <div className="flex items-end gap-[3px]" style={{ height: 28 }}>
      {delays.map((d, i) => (
        <div
          key={i}
          className="w-[5px] rounded-sm bg-teal-400"
          style={{
            height: '100%',
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 overflow-hidden">
      ...
      <div className="flex items-center gap-4 p-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-zinc-500 mt-0.5">Usually takes 10–20 seconds</p>
        </div>
      </div>

      {/* Phase stepper */}
      <div className="border-t border-zinc-800/60 px-5 py-3 flex flex-col gap-2">
        {PHASES.map((p, i) => {
            ...
            <div key={p.id} className="flex items-center gap-3">
              ...
              <span
                className={`text-xs transition-colors duration-300 ${
                  isDone ? 'text-zinc-500' : isCurrent ? 'text-white font-medium' : 'text-zinc-600'
                }`}
              >
                {p.label}
              </span>
